...
public class TinhToan extends UnicastRemoteObject implements ITinhToan{
    ...
    public ArrayList<SachBean> getS() throws RemoteException{
        ...
        try{
            ...
            while(rs.next()){
                ds.add(new SachBean(rs.getString("MaSach"), rs.getString("TenSach"), rs.getLong("SoLuong"), 
                        rs.getLong("Gia"), rs.getString("MaNhaXuatBan")));
            }
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    public ArrayList<NhaXuatBanBean> getNXB() throws RemoteException{
        ...
        try {
            ...
            ResultSet rs = cmd.executeQuery();
            while(rs.next()){
                ds.add(new NhaXuatBanBean(rs.getString("MaNhaXuatBan"), rs.getString("TenNhaXuatBan"), TbThanhTien(rs.getString("MaNhaXuatBan"))));
            } 
        } catch (Exception e) {
            e.printStackTrace();
        }
